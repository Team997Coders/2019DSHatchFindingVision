		...
		static void Main(string[] args)
		{
			...
			if (dev != null)
			{
				...
				if (new FilterGraph() is IFilterGraph2 graphBuilder)
				{
					...
					int cameraControlValue;

					// Get the current focus settings from the webcam
					_camera.Get(CameraControlProperty.Focus, out cameraControlValue, out cameraControlFlags);
					...
					// Get the current exposure settings from the webcam
